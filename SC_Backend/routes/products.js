  ...
  try {
    ...
    const products = await query(`
      SELECT 
        ...
        p.is_featured, p.stock_quantity, p.allergens, p.reviews, p.created_at,
    ...
    console.error('Get products error:', error.sqlMessage || error.message);
    res.status(500).json({ success: false, message: 'Server error getting products' });
  }
});
...
router.get('/featured', async (req, res) => {
  try {
    ...
    const products = await query(`
      SELECT 
        p.id, p.name, p.description, p.price, p.image_url, p.stock_quantity,
        c.name as category_name
      FROM products p
      LEFT JOIN categories c ON p.category_id = c.id
      WHERE p.is_featured = TRUE AND p.is_available = TRUE
    ...
    console.error('Get featured products error:', error.sqlMessage || error.message);
    res.status(500).json({ success: false, message: 'Server error getting featured products' });
  }
});
...
router.get('/search/:query', async (req, res) => {
  try {
    ...
    const products = await query(`
      SELECT 
        p.id, p.name, p.description, p.price, p.image_url, p.is_featured,
        c.name as category_name
      FROM products p
      LEFT JOIN categories c ON p.category_id = c.id
      WHERE (p.name LIKE ? OR p.description LIKE ? OR c.name LIKE ?)
    ...
    console.error('Search products error:', error.sqlMessage || error.message);
    res.status(500).json({ success: false, message: 'Server error searching products' });
  }
});
...
router.get('/:id', optionalAuth, async (req, res) => {
  try {
    ...
    console.error('Get product error:', error.sqlMessage || error.message);
    res.status(500).json({ success: false, message: 'Server error getting product' });
  }
});

...
const { query } = require('../config/database'); // CommonJS import
...
router.get('/orders', async (req, res) => {
  try {
    ...
    // Fetch all orders without limit/offset
...
router.get('/users', async (req, res) => {
  try {
    ...
    const countResult = await query(`SELECT COUNT(*) as total FROM users ${whereClause}`, safeParams(params));
    ...
    res.status(500).json({ success: false, message: 'Server error getting users', error: error.message });
  }
});
...
router.get('/contact-messages', async (req, res) => {
  try {
    ...
    const countResult = await query(`SELECT COUNT(*) as total FROM contact_messages ${whereClause}`, safeParams(params));

...
//         gsap.fromTo('.order-management-container', 
//           { opacity: 0, y: 20 },
...
//               <div key={order.id} className="order-card border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
//                 {/* Order Header */}
...
//                     <p className="text-gray-900">{order.customerInfo?.name || 'N/A'}</p>
//                   </div>
//                   <div>
//                     <p className="text-sm font-medium text-gray-500">Email</p>
//                     <p className="text-gray-900">{order.customerInfo?.email || 'N/A'}</p>
//                   </div>
//                   <div>
//                     <p className="text-sm font-medium text-gray-500">Phone</p>
//                     <p className="text-gray-900">{order.customerInfo?.phone || 'N/A'}</p>
//                   </div>
//                   <div>
...
const OrderManagement = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      try {
        const res = await getAllOrders();
        setOrders(res.data || []); // ensure array
      } catch (err) {
  ...
  const filteredOrders = safeOrders
    ...
    .sort((a, b) => {
      switch (sortBy) {
        ...
        case "total":
          return (b.total_amount || b.total) - (a.total_amount || a.total);
        case "status":
          return (a.status || "").localeCompare(b.status || "");
        default:
          return 0;

...
const WishlistModal = ({ isOpen, onClose }) => {
  ...
  const { user } = useAuth(); // Get user from AuthContext for the token

  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);

  // Fetch wishlist items when the modal opens
  useEffect(() => {
    const fetchWishlist = async () => {
      if (!isOpen || !user || !user.token) return; // Only fetch if modal is open and user is logged in
    ...
    fetchWishlist();
  }, [isOpen, user]); // Refetch when modal opens or user changes
  ...
  const clearWishlist = () => {
    // Note: Clearing wishlist via API would be a separate endpoint
    // For now, this only clears local state. Implement API call if needed.
    setItems([]);
  };
  ...
  if (loading) {
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[80vh] overflow-hidden p-8 text-center">
          Loading wishlist...
        </div>
      </div>
    );
  ...
  if (error) {
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[80vh] overflow-hidden p-8 text-center text-red-600">{error}</div>
      </div>
    );}
          ...
          {items.length === 0 ? (
            ...
            <div className="p-6">
              ...
              {items.length > 0 && (
                <div className="flex justify-end mb-4">
                  <button
                    onClick={clearWishlist} // This currently only clears local state
                    className="text-red-600 hover:text-red-700 font-medium transition-colors"
                  >
                    <i className="ri-delete-bin-line mr-1"></i>
                    Clear All

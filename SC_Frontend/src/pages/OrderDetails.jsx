...
const OrderDetails = () => {
  ...
  const getStatusSteps = () => {
    ...
    const currentIndex = order?.status ? statusOrder.indexOf(order.status) : -1;
  ...
  const handleStatusUpdate = (newStatus) => {
    if (canUpdateStatus()) {
      // TODO: hook to backend update API
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 to-orange-50 py-26 px-4 sm:px-6 lg:px-8">
      <div className="order-details-container max-w-4xl mx-auto">
        ...
        <div className="order-section bg-white rounded-2xl shadow-xl p-6 sm:p-8 mb-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
            ...
            <div className="text-left sm:text-right">
              <span
                className={`inline-block px-4 py-2 text-sm font-medium rounded-full border ${getStatusColor(order.status)}`}
              >
                {order.status
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-6 sm:gap-0">
              {getStatusSteps().map((step, index) => (
                <div key={step.key} className="flex flex-col items-center flex-1 relative">
                  <div
                    className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center border-2 mb-3 ${
                      step.completed
        ...
        {/* Action Buttons */}
        <div className="order-section bg-white rounded-2xl shadow-xl p-6 sm:p-8 mt-8">
